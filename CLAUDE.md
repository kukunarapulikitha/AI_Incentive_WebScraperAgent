...
- `config/sources.py` — 17 sources, each tagged `applicable_states=["FL"]` or `["ALL"]`. Pipeline auto-skips sources whose states don't intersect the active region.
...
- `pipeline/run.py` — orchestrator. Dedupes by (program_name, state), writes CSV in `CSV_COLUMN_ORDER`.
...
## Sources Status (last checked 2026-05-05)

| Key | Priority | Status | Records | Issue |
|---|---|---|---|---|
| `dsire_fl` | P0 | Partial | 28-29 | Playwright gets list page (names only); amounts/eligibility missing → all review_needed |
...
- **DSIRE deep scraping**: List page gives program names but no details. Need per-program detail page fetch (e.g. `programs.dsireusa.org/system/program/{id}`). This requires a spider pattern — not yet implemented.
...
- **Target**: 150+ validated records for Phase 1. Current: ~45 with `florida_statewide` run.
...
- Adding a new state = add a region preset in `config/regions.py` + add state-tagged sources in `config/sources.py`.
